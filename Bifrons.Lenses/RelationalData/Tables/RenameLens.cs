...
public sealed class RenameLens : SymmetricTableDataLens
{
    ...
    private RenameLens(Relational.Tables.RenameLens tableLens, IEnumerable<ISymmetricColumnDataLens> columnDataLenses, Option<IEnumerable<Column>> identityColumns = default)
    ...
    public static Result<RenameLens> Cons(Relational.Tables.RenameLens tableLens, IEnumerable<ISymmetricColumnDataLens> columnDataLenses, Option<IEnumerable<Column>> identityColumns = default)
        => tableLens.ColumnLenses.All(cdl => columnDataLenses.Any(cdl2 => cdl.MatchesColumnNameLeft == cdl2.MatchesColumnNameLeft))
            ? Result.Success(new RenameLens(tableLens, columnDataLenses, identityColumns))
